...
// void WifiSlave::onReceiveCallback(const uint8_t* mac, const uint8_t* buf, size_t count, void* arg) {
//     Serial.println("receiving messages");
//     WifiSlave* instance = static_cast<WifiSlave*>(arg); // Cast to instance
//     instance->processMessage(mac, buf, count); // Call the non-static method
// }
...
// void onDataReceive(const uint8_t *mac, const uint8_t *incomingData, unsigned int len, void* arg) {
//     // Print the MAC address of the sender
//     //Serial.printf("Message from %02X:%02X:%02X:%02X:%02X:%02X\n", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);

//     // Check if the received data has any content
//     if (len > 0) {
//         // First byte is the message type
//         uint8_t messageType = incomingData[0];

//         if (messageType == 0) {  // 0 indicates a vector
//             // Second byte is the condition
//             uint8_t condition = incomingData[1];
//             if (condition == 1) {
...
//             size_t vectorSize;
//             memcpy(&vectorSize, incomingData + 2, sizeof(size_t));

//             // Print the vector size
//             Serial.printf("Received vector of size: %zu\n", vectorSize);

//             // Print the vector values
//             Serial.print("Vector values: ");
//             for (size_t i = 0; i < vectorSize; i++) {
//                 int value;
//                 memcpy(&value, incomingData + 2 + sizeof(size_t) + i * sizeof(int), sizeof(int)); // Read each integer
//                 Serial.print(value);
//                 Serial.print(" ");
//             }
//             Serial.println();
...
//             // Read the integer after the first byte
//             int receivedInteger;
//             memcpy(&receivedInteger, incomingData + 1, sizeof(int));
...
// void WifiSlave::processMessage(const uint8_t* mac, const uint8_t* buf, size_t count) {
//     Serial.printf("Message from %02X:%02X:%02X:%02X:%02X:%02X\n", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
...
//             size_t vectorSize;
//             // memcpy(&vectorSize, buf + 1, sizeof(size_t));
//             memcpy(&vectorSize, buf + 2, sizeof(size_t));
...
//             for (size_t i = 0; i < vectorSize; i++) {
//                 int value;
//                 // memcpy(&value, buf + sizeof(uint8_t) + sizeof(size_t) + i * sizeof(int), sizeof(int));
//                 memcpy(&value, buf + sizeof(uint8_t) + sizeof(uint8_t) + sizeof(size_t) + i * sizeof(int), sizeof(int));
